...
def _run_migration_peak(csv: Path, base: Path, force_pandas: bool) -> tuple[int, float]:
    ...
    Se aísla en subproceso porque el pico de RSS se mide a nivel de proceso;
    un proceso por medición da lecturas limpias. Con force_pandas se desactiva
    el path streaming para medir el path clásico (carga completa).

    Multiplataforma: en lugar de `resource` (POSIX-only) se muestrea el RSS con
    psutil cada 10 ms en un hilo daemon y se conserva el máximo observado. Esto
